function setupCanvas(canvas) {
    ...
    with width:100% vs window.innerWidth; */
    // Give the canvas pixel dimensions of their CSS
    // size * the device pixel ratio.
    // Fix for hiding url bar on phone by urlbar.
    var urlbar = 70;
    canvas.width = (window.innerWidth + urlbar) * dpr;
    canvas.height = (window.innerHeight + urlbar) * dpr;
    canvas.style.width =  (window.innerWidth + urlbar)+"px";
...
function init() {
    ...
    let numberOfEyes = Math.random()*200+50;
    ...
    while (eyes.length < numberOfEyes && counter < protection) {
        let eye = {
            ...
            radius: Math.floor(Math.random() * 160) + 10
        };
        overlapping = false;
...
window.addEventListener("resize", function () {
    ...
    if(canvas.height < window.innerHeight*dpr || canvas.width < window.innerWidth*dpr){
        this.console.log(canvas.height+" < "+window.innerHeight*dpr+"\n"+
